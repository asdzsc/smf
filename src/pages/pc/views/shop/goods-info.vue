...
<<<<<<< HEAD
  <div class="goodsInfo">
      ...
      <a-breadcrumb separator="》">
        ...
        <a-breadcrumb-item v-if="goodsColumnList.length > 0">
          <template v-for="(item, index) in goodsColumnList">
            <a :key="item.id" :href="`/#/shop/goodsList?cid=${item.id}`">
              {{ item.name }}
            </a>
            <label
              :key="`label${item.id}`"
              v-if="index !== goodsColumnList.length - 1"
              >/</label
            >
          </template>
        </a-breadcrumb-item>
    ...
    <div class="goodsSpec">
              ...
              <td>
                <div class="carousel">
                  <a-carousel
                    ...
                    :autoplay="true"
                    :dots="false"
                    :afterChange="goodsImgChange"
                  >
                    <div
                      v-for="(item, index) in imgCarouselList"
                      :key="'imgCarousel' + index"
                      class="goodsCarouselGroup"
                    >
                      <div
                        v-for="imgUrl in item"
                        :key="imgUrl"
                        class="carouselImg"
                        @mousemove="setShowGoodsImg(imgUrl)"
                        @click="showGoodsImgModal(imgUrl)"
                      >
      ...
      <div class="goodsSpecRight">
        ...
        <div class="goodsPrice">
          ...
          ><span class="price">{{ goodsPriceText }}</span>
        ...
        <div
          v-if="model.goodsType === '1' || model.goodsType === '3'"
          class="quantity"
        >
        ...
        <div v-if="model.goodsType === '1'" class="buyCont">
          ...
          <div @click.stop="openConfirmOrder()" class="buyRight">立即购买</div>
        </div>
        <div v-else class="buyCont" style="width: 110px">
          <div
            @click.stop="openConfirmOrder()"
            class="buyRight"
            style="flex: 1"
          >
            立即购买
    ...
    <goodsShowImg
      v-if="showImgModal"
      ref="goodsShowImg"
      @onClose="goodsShowImgClose"
    ></goodsShowImg>
  </div>
</template>
<script>
import { goodsInfo, addCart } from "@/pages/pc/api/shop";
...
export default {
  components: {
    ...
    goodsComment: () =>
      import("@/pages/pc/views/shop/components/goods-comment.vue"),
    goodsShowImg: () =>
      import("@/pages/pc/views/shop/components/goods-showimg.vue"),
  },
  data() {
    return {
