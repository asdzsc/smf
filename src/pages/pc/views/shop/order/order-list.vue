<template>
	<div class="member">
				...
				<a-spin size="large" tip="加载中..." :spinning="loading">
					...
					<div v-for="item in model.list" :key="item.id" class="card">
						...
						<table style="width: 100%; border-collapse:collapse;">
							<tr>
								<td style="width: 800px;">
									<a-row v-for="goodsItem in item.goodsList" :key="goodsItem.id" type="flex" class="cardBody">
										...
										<a-col flex="auto" class="orderGoodsTitle">
											<div @click="openGoodsInfo(goodsItem)" style="color: #2483D5; cursor: pointer;">{{ goodsItem.title }}</div>
								...
								<td class="operation" style="width: 110px;">
									...
									<div v-if="item.isFeedback === '1'" @click="showFeedback(item)" class="comment">
										【查看服务反馈】
									</div>
									<div v-if="item.isComment === '1'" @click="showComment(item)" class="comment">
										【查看我的评价】
									</div>
									<div v-if="item.orderStatus === '5' && item.isComment === '0'" @click="saveComment(item)" class="comment">
										【点击评价】
									</div>
								</td>
							</tr>
						...
						<a-row type="flex" justify="end" class="totalMoney">
							...
							<a-col>
								实付款：<span class="money">{{ formatMoney(item.payment) }}</span>
							</a-col>
						</a-row>
					</div>
				</a-spin>
		...
		<!-- 评论信息 -->
		<orderCommentList v-if="showCommentList" ref="orderCommentList" :orderId="orderId" commentType="1" @onClose="closeOrderComment"></orderCommentList>
		<!-- 反馈信息 -->
		<orderCommentList v-if="showFeedbackList" ref="orderFeedbackList" :orderId="orderId" commentType="2" @onClose="closeOrderFeedback"></orderCommentList>
		<!-- 反馈信息 -->
		<orderComment v-if="showSaveComment" ref="orderComment" :shopOrder="shopOrder" @onClose="closeSaveComment"></orderComment>
...
<script>
	...
	export default {
		components: {
			...
			orderCommentList: () => import("@/pages/pc/views/shop/order/components/order-comment-list.vue"),
			orderComment: () => import("@/pages/pc/views/shop/order/components/order-comment.vue")
		},
		data() {
			return {
