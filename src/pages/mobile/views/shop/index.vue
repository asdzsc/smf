	...
	<div class="index">
		...
		<van-tabs v-model="active" color="#004930" scrollspy sticky>
			...
			<van-tab :title="'礼仪服务'">
				...
				<van-swipe @change="onChange" autoplay="5000" v-if="lyfwList.length > 0">
					<van-swipe-item v-for="item in lyfwList" :key="item.id" @click="openGoodsInfo(item)">
						<div class="lyfw">
							<img @click="openGoodsInfo(item)" :src="getGoodsImg(item.cover)" alt="">
							<div class="cont">
							...
							<div class="buy">
								...
								<div class="btn" @click.stop="openGoodsInfo(item)">立即购买</div>
							</div>
						</div>
					</van-swipe-item>
					<template #indicator>
...
<style lang="less" scoped>
	.index {
		...
		.contHead {
			...
			margin: 0.3rem .3rem .21rem .14rem;
