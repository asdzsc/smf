	...
	<div class="orderInfo">
		...
		<div class="title">
			<div class="link">
				<a href="/mobile/#/shop/order/orderList">服务订单</a>
				<span>
					<van-icon name="arrow" />订单详情</span>
			</div>
		</div>
		<div v-if="model.orderStatus === '1'" style="width: 90%;margin: 10px auto;">
			<div slot="description">
				<van-icon style="vertical-align: middle;margin-right: .1rem;" name="warning-o" color="#faad14" size=".5rem" />
				<p style="display: inline;line-height: .5rem;">订单当前状态为【待支付】 ,
					<span style="font-size: .16rem; font-weight: 700; color: #d54040;">
						{{ dateDown }}
					</span>
					后订单将自动关闭</p>
			</div>
...
<script>
	...
	export default {
		...
		components: {
			...
			orderInfoCont: () => import("@/pages/mobile/views/shop/order/components/order-info-cont.vue"),
		},
		mounted() {
			this._getOrderInfo();
			this._orderLogList();
