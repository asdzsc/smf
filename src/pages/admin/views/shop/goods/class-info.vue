...
export default {
  ...
  mounted() {
    ...
    this.model.columnType = this.columnType || "1";
    this.loadData();
  },
  methods: {
    loadData() {
      this.getModule();
      this.visible = true;
      this.title = this.model.id ? "编辑栏目" : "添加栏目";
      if (this.model.id) {
        getGoodsColumnInfo({ id: this.model.id }).then(res => {
          if (res.code === 0) {
    ...
    onSubmit() {
      saveGoodsColumnInfo(this.model).then(res => {
        if (res.code === 0) {
