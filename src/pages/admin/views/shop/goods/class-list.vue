...
export default {
  components: {
    goodsColumnInfo: () => import("@/pages/admin/views/shop/goods/class-info")
  },
  data() {
    return {
      columnType: "1",
  ...
  methods: {
    ...
    getList() {
      ...
      getGoodsColumnList({ columnType: this.columnType }).then(res => {
        this.loading = false;
        if (res.code === 0) {
    ...
    setStatus(record) {
      this.$confirm({
        ...
        onOk() {
          ...
          setGoodsColumnStatus({ id: record.id, status: status }).then(res => {
            if (res.code === 0) {
    ...
    saveSort() {
      saveGoodsColumnSort(this.newsColumnList).then(res => {
        if (res.code === 0) {
    ...
    del(list) {
      ...
      this.$confirm({
        ...
        content: "",
        onOk() {
          delGoodsColumn({ ids: list }).then(res => {
            if (res.code === 0) {
              vm.getList();
            }
          });
