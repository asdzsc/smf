...
=======
	<a-card>
		<a-row>
			<a-col :span="4">
				<a-tree :treeData="treeData" :defaultSelectedKeys="selectedKeys" :expandedKeys="expandedKeys" @expand="onExpandTree"
				 @select="onSelectTree" />
			</a-col>
			<a-col :span="20">
				<div class="operator">
					<a-row type="flex" justify="start">
						<a-col style="padding-right:10px;">
							<a-button @click="saveSort()" type="primary" icon="sort-descending">
								保存排序
							</a-button>
							&nbsp;
					 ...
					 :loading="loading" @change="onChange">
						...
						<template slot="newsTitle" slot-scope="record">
							<div style="line-height: 25px;">
								...
								<div>
									商品价格：<span style="color:#d54040;">{{ goodsPriceText(record) }}</span>
									<span v-if="record.goodsType==='3'"> / 年</span>
								</div>
							</div>
						</template>
		...
		<goodsInfo v-if="showInfo" :id="infoId" ref="goodsInfo" @onClose="infoClose"></goodsInfo>
...
<script>
	...
	export default {
		...
		mounted() {
			this.pagination.columnId = this.$route.query.classId ? this.$route.query.classId : ''
			this.$nextTick(() => {
				if (this.pagination.columnId) {
					this.selectedKeys = [this.pagination.columnId]
