<template>
	<a-card>
		...
		<info v-if="showInfo" :id="infoId" :bannerType="pagination.bannerType" @onClose="infoClose"></info>
...
<script>
	import {
		...
		delSysBanner,
		saveSysBannerSort
	} from "@/pages/admin/api/sys/sysbanner";

	export default {
		components: {
			showImg: () => import("@/pages/admin/components/show-img/show-img.vue"),
			info: () => import("@/pages/admin/views/sys/banner/info")
		},
		data() {
			return {
		...
		watch: {
			$route(to, from) {
			  if (this.$route.query.bannerType) {
			  	this.pagination.bannerType = this.$route.query.bannerType
			  }
			  this.getList();
			}
		},
		...
		methods: {
			...
			getList() {
				...
				getSysBannerList(pagination).then(res => {
					this.loading = false;
					if (res.code === 0) {
			...
			saveSort() {
				saveSysBannerSort(this.list).then(res => {
					if (res.code === 0) {
