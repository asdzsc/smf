		...
		<a-form-model :model="model" :label-col="{ span: 4 }" :wrapper-col="{ span: 18 }">
			<a-form-model-item label="所属栏目">
				<a-tree-select v-model="model.columnIds" style="width: 600px;" :dropdown-style="{ maxHeight: '400px', overflow: 'auto' }"
				 :tree-data="treeData" placeholder="请选择新闻栏目" tree-default-expand-all allow-clear multiple @change="changeColumn">
				</a-tree-select>
			</a-form-model-item>
			<a-form-model-item label="标题">
				<a-input v-model="model.title" placeholder="请输入新闻标题" style="width:600px;" />
			</a-form-model-item>
          ...
          直播视频封面为直播未开始或暂停时显示的宣传页，推荐尺寸为：9：16，支持JPG、PNG格式，图片大小不超过4M
			...
			<a-form-model-item label="作者">
				<a-input v-model="model.author" placeholder="请输入作者" style="width:600px;" />
			</a-form-model-item>
			<a-form-model-item label="发布时间">
				<a-date-picker show-time placeholder="请选择新闻发布时间" @change="onChangeDate" v-model="model.publishTimeDate" style="width:200px;"
				 inputReadOnly />
			</a-form-model-item>
			<a-form-model-item label="简介">
				<a-textarea v-model="model.intro" placeholder="新闻简介,不填写默认截取内容" :auto-size="{ minRows: 5, maxRows: 5 }" />
			</a-form-model-item>

			<a-form-model-item label="内容">
				<script id="articleEditor" type="text/plain" style="width:100%;height:500px;"></script>
			</a-form-model-item>
		</a-form-model>
